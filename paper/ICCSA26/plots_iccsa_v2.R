...
# ── Tema base — estilo conferência acadêmica (Springer LNCS / IEEE) ──────────
#
# Princípios:
#   - Fundo branco puro, sem painel colorido
#   - Eixos pretos com ticks visíveis (como em figuras de paper)
#   - Grid horizontal em cinza muito claro — apenas guia de leitura
#   - Sem borda de legenda; legenda integrada ao espaço do plot
#   - Fonte e tamanhos compatíveis com 1 coluna (~8 cm) e 2 colunas (~17 cm)
#   - strip.text para facets com borda fina — identifica GPU claramente
theme_iccsa <- function(base_size = 9) {
  theme_bw(base_size = base_size) +
    theme(
      ...
      # Ticks: pretos, curtos, visíveis nos dois eixos
      axis.ticks        = element_line(color = "black", linewidth = 0.35),
      axis.ticks.length = unit(3, "pt"),

      # Linhas de eixo: borda do painel via panel.border (theme_bw)
      panel.border      = element_rect(color = "black", linewidth = 0.5, fill = NA),
      panel.background  = element_rect(fill = "white", color = NA),

      # Grid: apenas horizontal, cinza claro tracejado — discreto
      panel.grid.major.y = element_line(color = "#DDDDDD", linewidth = 0.35,
                                        linetype = "dashed"),
      panel.grid.major.x = element_blank(),
      panel.grid.minor   = element_blank(),

      # Legenda no topo, sem caixa, compacta
      legend.position    = "top",
      ...
      # Facets (strip): borda fina, fundo cinza muito claro, texto bold
      strip.background   = element_rect(fill = "#F0F0F0", color = "black",
                                        linewidth = 0.5),
      strip.text         = element_text(size = base_size, face = "bold",
                                        color = "black", margin = margin(3, 3, 3, 3)),
...
theme_iccsa_hbar <- function(base_size = 9) {
...
fig4_heatmap_ratio <- function(df) {
  ...
  save_fig(p, "fig4_heatmap_ratio", 3.8 * length(gpus), 3.4)
}
