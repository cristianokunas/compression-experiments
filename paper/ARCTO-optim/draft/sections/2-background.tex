...
The AMD software ecosystem centers on
ROCm~\cite{rocm}, an open-source platform that includes the HIP
programming model. HIP source code is near-syntactically identical
...
documented in our prior characterization work
\cite{kunas2026iccsa}: while NVIDIA has \nvcomp{} as a
production-ready library, the only AMD-side counterpart is
...
The MI50, MI210, and MI300X experiments use the Grid'5000
testbed~\cite{grid5000} at the Luxembourg site (vianden cluster),
which provides exclusive access to single-tenant compute nodes.
The RX~7900~XT experiments run on the PCAD\footnote{
\url{https://pcad.inf.ufrgs.br/}} laboratory at UFRGS on a
Threadripper PRO 5965WX host with 256~GB DDR4 and PCIe~Gen4 x16
host interconnect. Both platforms run the same container image,
differing only in the architecture-specific build target.
...
benchmark~\cite{fletcher2009tti}, the same workload used in our
ICCSA characterization~\cite{kunas2026iccsa}. The dataset is
...
RX 7900 XT (20 GB) & \checkmark & \checkmark & \checkmark & \checkmark & ${\sim}$ & --- \\
\bottomrule
\end{tabular}
\end{table}
